...
export async function getLatestInterviews(
  ...
  try {
    const query = adminDb.collection("interviews").orderBy("createdAt", "desc");
    ...
    if (error.code === 9) {
      console.log(
        "⚠ Firestore index required for getLatestInterviews query. Returning empty array."
      );
    } else {
...
export async function getInterviewsByUserId(
  ...
  try {
    ...
    if (error.code === 9) {
      console.log(
        "⚠ Firestore index required for getInterviewsByUserId query. Returning empty array."
      );
    } else {
